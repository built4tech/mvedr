...
# MVEDR doesn't support other encode but UTF-8, transforming the original message (UTF16) of the event to UTF 8 requires
# the creation of a temporal file. Additionally to the event message the collector also shows a quick hint.

# https://digital-forensics.sans.org/media/DFPS_FOR508_v4.3_12-18.pdf

# Hint of the events to be capure
$eventHint = @{}
...
#$systemlogs   = Get-EventLog -Log $eventlogtype -After (Get-Date).AddDays(-1) -ErrorAction SilentlyContinue | where {$_.eventID -in 7034,7035,7036,7040,7045}|Sort-Object -Property Timecreated -Descending 
...
foreach ($log in $systemlogs){
    ...
    $username = $log.UserName -replace '\\','-' # MV EDR doesn't support the backslash character
    $source   = $log.Source
    $id       = $log.EventID    
    $msg      = $log.Message 
    $msg      = $msg -replace ',','.'
...
$seclogs   = Get-EventLog -Log $eventlogtype -After (Get-Date).AddDays(-1) -ErrorAction SilentlyContinue | where {$_.eventID -in 4624,4648,4672,4697,4698,4699,4700,4701,4702,5140}|Sort-Object -Property Timecreated -Descending
...
foreach ($log in $seclogs){
    ...
    $msg      = $msg -replace ',','.' # In the case the msg has a comma a replace it with a dot as MvEDR requires a comma separated table
